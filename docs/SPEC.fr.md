...
- **Support Xiaomi MiMo-V2.6** : MiMo-V2.6-Flash, MiMo-V2.6-Pro et MiMo-V2.6-Pro-UltraSpeed ajoutés comme modèles du fournisseur Direct MiMo. Les trois disposent d'une fenêtre de contexte de 1 000 000 tokens et d'un support multimodal natif (texte, image, vidéo). Routes par défaut intégrées : Opus 5 → `mimo-v2.6-pro` (Thinking), Sonnet 5 → `mimo-v2.6-pro` (Normal), Haiku 4.5 → `mimo-v2.6-flash` (Thinking). MiMo utilise uniquement le basculement Normal/Thinking — aucun niveau d'effort de raisonnement (`supportsReasoningEffort: false`).
- **Compatibilité ascendante MiMo V2.5** : Les modèles hérités `mimo-v2.5`, `mimo-v2.5-pro` et `mimo-v2.5-pro-ultraspeed` restent statiquement reconnus avec leurs capacités d'origine. Les routes par défaut historiques non modifiées migrent automatiquement vers V2.6 au démarrage ; les cibles de routes, modes thinking et paramètres default_model personnalisés par l'utilisateur sont préservés. Les configurations héritées utilisant `model_map` sans objet `models` résolvent les capacités par route depuis le résolveur de capacités statique (`try_resolve_static_model_capabilities`) plutôt que de se rabattre sur les indicateurs globaux du fournisseur, garantissant que les routes V2.6 reçoivent la bonne capacité vidéo même dans les configurations héritées.
...
- **Registre des capacités de contexte** : `model_context_windows.json` est un registre statique des capacités de contexte connues couvrant les modèles intégrés des fournisseurs directs (DeepSeek, MiniMax, Kimi, MiMo V2.6 et V2.5) et les modèles OpenRouter intégrés (Poolside, Tencent, InclusionAI, StepFun, OpenAI GPT-5.6). Les modèles OpenRouter personnalisés inconnus restent des cibles de routage valides mais rapportent la gestion du contexte comme Incomplete jusqu'à ce que des métadonnées soient ajoutées ou qu'un mode manuel soit configuré.
