...
モデル単位の `supports_vision` / `supports_video` フラグで判定。非対応モデルに画像/動画が送られた場合は 400 エラー。
...
24 commands. See Japanese section for full list.
...
Model-based routing. API key validation uses 2-pass approach since v0.5.0:
...
See Japanese section for full schema. Key fields:
- `active_provider`: Selected provider ID
- `providers.<id>.models`: gateway model → upstream model mapping
- `models.<gm>.upstream_model`: Actual model name sent to upstream
- `models.<gm>.thinking`: `"disabled"` injects thinking suppression
- `models.<gm>.visible`: Whether to expose in `/v1/models`
