...
Laguna S 및 Laguna XS는 OpenRouter 추론 변환 규칙을 사용합니다.

Anthro Bridge는 또한 응답이 추론 콘텐츠만 생성하고 사용 가능한 텍스트나 도구 호출이 없는 상태에서 출력 토큰 한도에 도달하는 실패 패턴을 감지합니다. 감지되면 이벤트가 로그에 기록되어 사용자가 출력 한도를 조정하거나 thinking을 비활성화하거나 다른 모델을 선택할 수 있습니다.
