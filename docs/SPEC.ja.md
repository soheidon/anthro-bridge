...
- **推論エフォート**: DeepSeek V4 Pro は Thinking モードで High / Max、V4 Flash は Low / High / Max に対応。Normal モードでは推論エフォートを無効化。V4 Pro ルートに保存されたレガシー `low`/`medium` は起動時に `high` へ移行。
...
設定画面 (=):
  ...
  | プロバイダごとのAPIキー管理           |
  | ┌─────────────────────────────────────┐|
  | │ DeepSeek   ▾  (クリック/Enter/Space │|
  | │ で展開)                             │|
  | │  ├ Opus:    [deepseek-v4-pro ▾]     │|
  | │  ├ Sonnet:  [deepseek-v4-pro ▾]     │|
  | │  ├ Haiku:   [deepseek-v4-flash ▾]   │|
  | │  ├ Thinking Mode: [normal ▾]        │|
  | │  ├ Reasoning Effort: [high ▾]       │|
  | │  ├ Env var: [DEEPSEEK_API_KEY]      │|
  | │  └ API key: [sk-...] [Save]         │|
  | └─────────────────────────────────────┘|
  | model/thinking/reasoning は自動保存    |
  | API キーは明示的 Save ボタン           |
  | K3: reasoning_effort=max 固定表示      |
  +- Model Pricing ------------------------+
  | 折りたたみ式料金表 (USD/1Mトークン)    |
  | 入力/出力/キャッシュ単価を一覧表示      |
...
`models` エントリの `thinking_mode` に従い4つの動作モードを制御:
- `disabled`: 常に `{"type": "disabled"}` を注入
- `thinking`: ユーザー未指定時に `{"type": "enabled", "budget_tokens": 10000}` を注入
- `thinking_only`: `thinking` パラメータなしでは 400 エラー。自動注入必須
- `normal`: 介入なし

#### K3 特殊処理 (v0.11.0)

Kimi K3 は `thinking` パラメータ非対応。`suppressThinkingParameter: true` + `forcedReasoningEffort: "max"` により:
- リクエストから `thinking` パラメータを除去
- `reasoning_effort: "max"` を注入
- temperature/top_p 等の固定パラメータを除去
